...
private static ArrayList<ArrayList<Integer>> activeTerms;
...
//        }
        ...
        for (int j = 0; j < numberAttributes; j++) {
            ...
            if (infoAttributes.get(j) == 1) { //Numeric attribute
                ...
                /*else if (individual[pos + 1] == 3){//attribute = attribute
                     ...
                     } else if (individual[pos + 1] == 4){//attribute != attribute
                    
                     }*/ //Pra atributos categoricos tem que pensar ainda...
            }
    ...
    /* ========================================================================================
         * This code below randomly initiates the individuals 
         * ========================================================================================
     */
 ...
 /*while (population.size() < Parameters.getNumberInitialRules()) {
         ...
         //Randomly choose values for the clausule depending on the operator chosen
         //The values chosen will be values among the instances values
         if (individual[pos + 1] == 0 || individual[pos + 1] == 1) {
         individual[pos + 2] = 0.0;
         individual[pos + 3] = getNumericValue(j);

         } else if (individual[pos + 1] == 2 || individual[pos + 1] == 3) {
         individual[pos + 2] = getNumericValue(j);
         individual[pos + 3] = 0.0;

         } else {
         double[] lowerHigherValue = getLowerHigherValue(j);
         individual[pos + 2] = lowerHigherValue[0];
         individual[pos + 3] = lowerHigherValue[1];
         }
         ...
         ArrayList<Integer> atomicAttributes = new ArrayList<Integer>();

         for (int k = 0; k < Datasets.getCategoricMapping().get(posAttribute).size(); k++) {
         if (Datasets.getCategoricMapping().get(posAttribute).get(k).length == 1) {
         atomicAttributes.add(k);
         }
         }

         individual[pos + 2] = getCategoricValue(atomicAttributes);
         individual[pos + 3] = 0.0;
         ...
         for (int k = 0; k < Datasets.getCategoricMapping().get(posAttribute).size(); k++) {
         if (Datasets.getCategoricMapping().get(posAttribute).get(k).length > 1) {
         setOfAttributes.add(k);
         }
         }

         individual[pos + 2] = getSetOfCategoricValues(setOfAttributes);
         individual[pos + 3] = 0.0;
...
/* ===========================================================
     ...
     * returns its index
     * =========================================================== */
public static int getPosAttributeLowerValue(int posExample, int posAttribute, double attributeValue) {
    ...
    for (int i = 0; i < numberAttributes; i++) {
        if (Datasets.getInfoAttributes().get(i) == 1 && i != posAttribute) {
            ...
            if (Datasets.getTokenMissingValue().equals(datasetTrain.get(posExample)[i]) == true) {
                searchValue = Datasets.getMeanValues().get(0)[i];
            } else {
                searchValue = Double.parseDouble(datasetTrain.get(posExample)[i]);
            }
...
private double getNumericValue(int numAttribute) {
    ...
    Random generator = new Random();
    int numInstance = generator.nextInt(datasetTrain.size());
    String stringValue = datasetTrain.get(numInstance)[numAttribute];

    while (stringValue.equals(Datasets.getTokenMissingValue()) == true) {
        numInstance = generator.nextInt(datasetTrain.size());
        stringValue = datasetTrain.get(numInstance)[numAttribute];
    }
...
/*
     ...
     * 
 */
public static ArrayList<double[]> getPopulation() {
    return population;
}

public static ArrayList<ArrayList<Integer>> getActiveTerms() {
    return activeTerms;
}
}
